...
/*
//-----------------------------------------------------------------------------
// TestRESTSubReqPolicyChangeAndSubDelOk  This case will not work before interface specification is fixed
//
//   stub                             stub
// +-------+        +---------+    +---------+
// | xapp  |        | submgr  |    | e2term  |
...
func TestRESTSubDelReqRetransmission(t *testing.T) {
	...
	// Retry del. Submgr will reply OK for this even processing the first request is still ongoing
	xappConn1.SendRESTSubsDelReq(t, &restSubId)
...
func TestRESTSubReqDelReq(t *testing.T) {
	...
	// Del. This will fail as processing of the subscription is still ongoing in submgr. Deletion is not allowed before
...
//-----------------------------------------------------------------------------
// TestRESTSubReqAndSubDelOkTwoParallel This case has failed some times
//
//   stub       stub                                stub
// +-------+  +-------+           +---------+    +---------+
// | xapp2 |  | xapp1 |           | submgr  |    | e2term  |
...
func TestRESTSameSubsDiffRan(t *testing.T) {
	...
	xappConn1.WaitRESTNotification(t)
	e2termConn1.SendSubsResp(t, crereq, cremsg)
	e2SubsId2 := <-xappConn1.RESTNotification
	xapp.Logger.Info("TEST: REST notification received e2SubsId2=%v", e2SubsId2)
...
func TestRESTSubReqAsn1EncodeFail(t *testing.T) {
	...
	// Req
	params := xappConn1.GetRESTSubsReqReportParams(subReqCount, parameterSet, actionDefinitionPresent, actionParamCount)
	// Set invalid type of message
	params.SetTventTriggerTypeOfMessage(0)
	restSubId := xappConn1.SendRESTReportSubsReq(t, params)
	xappConn1.WaitRESTNotification(t)
	e2SubsId := <-xappConn1.RESTNotification
	xapp.Logger.Info("TEST: REST notification received e2SubsId=%v", e2SubsId)
