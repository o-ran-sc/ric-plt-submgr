...
func NewControl() *Control {
	...
	c := &Control{e2ap: new(E2ap),
		registry:   registry,
		tracker:    tracker,
		debugPrint: false, // Change this true if you need more logs
	}
	c.XappWrapper.Init("")
...
func (c *Control) RESTSubscriptionRequestHandler(stype models.SubscriptionType, params interface{}) (*models.SubscriptionResponse, error) {
	...
	case models.SubscriptionTypeReport:
		...
		xAppHttpEndPoint, xAppRmrEndpoint, err := xapptweaks.ConstructEndpointAddresses(*p.ClientEndpoint)
		if err != nil {
		...
		err = c.e2ap.FillReportSubReqMsgs(stype, params, &subReqList, restSubscription)
		if err != nil {
			xapp.Logger.Error("%s", err.Error())
	...
	case models.SubscriptionTypePolicy:
		...
		subReqList := e2ap.SubscriptionRequestList{}
		err = c.e2ap.FillPolicySubReqMsgs(stype, params, &subReqList, restSubscription)
		if err != nil {
			xapp.Logger.Error("%s", err.Error())
			c.registry.DeleteRESTSubscription(&restSubId)
			return nil, err
