...
func (c *Control) processSubscriptionRequests(trans *TransactionXapp, restSubscription *RESTSubscription, subReqList *e2ap.SubscriptionRequestList, xAppHttpEndPoint *string, xAppRmrpEndPoint *string, meid *string, restSubId *string, actionType uint64) {
	...
	for _, subReqMsg := range subReqList.E2APSubscriptionRequests {
		xapp.Logger.Info("Handle SubscriptionRequest. InstanceId = %v", subReqMsg.RequestId.InstanceId)

		subRespMsg, err := c.handleSubscriptionRequest(trans, &subReqMsg, xAppRmrpEndPoint, meid, restSubId, actionType)
		if err != nil {
			c.registry.DeleteRESTSubscription(restSubId)
			xapp.Logger.Error("Handle SubscriptionRequest failed. InstanceId = %v", subReqMsg.RequestId.InstanceId)
		} else {

			// Successfully processed InstanceId is stored for deletion
...
//
//-------------------------------------------------------------------
func (c *Control) FormatEndpointAddresses(clientEndpoint string) (string, string, error) {

	// xApp sends client endpoint is in this format: service-ricxapp-ueec-http.ricxapp:8080
	if i := strings.Index(clientEndpoint, ":"); i == -1 {
		err := fmt.Errorf("Incorrect ClientEndpoint = %s", clientEndpoint)
		xapp.Logger.Error("%s", err.Error())
		xapp.Logger.Info("Correct ClientEndpoint format is: service-ricxapp-xappname-http.ricxapp:8080")
		return "", "", err
	}

	// xApp's http client endpoint is in this format: service-ricxapp-ueec-http.ricxapp
	xAppHttpEndPoint := clientEndpoint
	i := strings.Index(xAppHttpEndPoint, ":")
	if i != -1 {
		xAppHttpEndPoint = xAppHttpEndPoint[0:i]
	}

	if i := strings.Index(clientEndpoint, "localhost"); i != -1 {
		// Assuming test address is used
		// Test client endpoint is in this format: localhost:13560
		return xAppHttpEndPoint, clientEndpoint, nil
	}

	// xApp's RMR client endpoint should be in this format: service-ricxapp-ueec-rmr.ricxapp:4560
	...
	if i := strings.Index(clientEndpoint, "http"); i != -1 {
		...
		err := fmt.Errorf("Incorrect ClientEndpoint = %s. http is missing", clientEndpoint)
		xapp.Logger.Error("%s", err.Error())
		return "", "", err
	}
	...
	if i := strings.Index(xAppRrmEndPoint, "8080"); i != -1 {
		...
		err := fmt.Errorf("Incorrect ClientEndpoint = %s. 8080 is missing", clientEndpoint)
		xapp.Logger.Error("%s", err.Error())
		return "", "", err
	}
...
func (c *Control) RESTSubscriptionRequestHandler(stype models.SubscriptionType, params interface{}) (*models.SubscriptionResponse, error) {
	...
	case models.SubscriptionTypeReport:
		...
		xAppHttpEndPoint, xAppRmrEndpoint, err := c.FormatEndpointAddresses(*p.ClientEndpoint)
		if err != nil {
			return nil, err
		}
		...
		go c.processSubscriptionRequests(trans, restSubscription, &subReqList, &xAppHttpEndPoint, &xAppRmrEndpoint, &p.Meid, &restSubId, actionType)
		// Respond to xapp
		return &subResp, nil
