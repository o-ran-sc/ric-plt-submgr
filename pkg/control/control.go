...
func (c *Control) handleSubscriptionRequest(params *xapp.RMRParams) {
	...
	transaction, err := c.tracker.TrackTransaction(newSubId, CREATE, *srcAddr, *srcPort, params, respReceived, forwardRespToXapp)
	if err != nil {
...
func (c *Control) handleSubscriptionResponse(params *xapp.RMRParams) {
	...
	if transaction.RespReceived == true {
		// Subscription Response timer already received
		return
	}
...
func (c *Control) handleSubscriptionDeleteFailure(params *xapp.RMRParams) {
	...
	transaction, err := c.tracker.RetriveTransaction(payloadSeqNum, DELETE)
	if err != nil {
		xapp.Logger.Info("SubDelFail: Failed to retrive transaction record. Dropping msg. Err %v, SubId: %v", err, params.SubId)
		return
	}
...
func (c *Control) handleSubscriptionDeleteRequestTimer(strId string, nbrId int, tryCount uint64) {
	...
	if err != nil {
		xapp.Logger.Info("handleSubDelTimer: Failed to retrieve transaction record. Err: %v, newSubId: %v", err, subId)
		return
	}
