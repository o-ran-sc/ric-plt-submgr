...
func (c *Control) SubscriptionHandler(params interface{}) (*models.SubscriptionResponse, error) {
	...
	if p.SubscriptionID == "" {
		if exists {
			restSubscription, _ = c.registry.GetRESTSubscription(prevRestSubsId, false)
		...
		xapp.Logger.Info("RestSubscription provided via REST request %s", restSubId)
...
func (c *Control) processSubscriptionRequests(restSubscription *RESTSubscription, subReqList *e2ap.SubscriptionRequestList,
	...
	defer restDuplicateCtrl.TransactionComplete(md5sum)
	...
	for index := 0; index < len(subReqList.E2APSubscriptionRequests); index++ {
		...
		if err != nil {
			...
			restDuplicateCtrl.SetMd5sumFromLastOkRequest(*restSubId, md5sum)
