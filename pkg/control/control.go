...
func (c *Control) handleE2TSubscriptionDeleteRequired(params *xapp.RMRParams) {
	...
	//TODO: Create RIC Subscription Delete Required Msg and Sent to xApp
	for _, subsDelRequ := range subscriptions {
		subsDelNotifyMsg := &e2ap.SubscriptionDeleteRequiredList{E2APSubscriptionDeleteRequiredRequests: subsDelRequ}
		mType, packedData, err := c.e2ap.PackSubscriptionDeleteRequired(subsDelNotifyMsg)
		if err != nil {
			xapp.Logger.Error("Unable to Encode RIC Subscription Delete Required Msg to xAPP :: %s", err.Error())
			continue
		}
		trans := &TransactionXapp{
			...
			XappKey: &TransactionXappKey{Xid: params.Xid},
		}
		c.sendSubsDelNotificationToxApp(subsDelRequ[0].InstanceId, trans)
	}
...
//-------------------------------------------------------------------
//	Sends Subscription Delete Required Msg Notification to xApp
//-------------------------------------------------------------------
func (c *Control) sendSubsDelNotificationToxApp(instanceId uint32, trans *TransactionXapp) {
	//TODO: Need to Check to send Notification Message to xApp after Subscription Delete
	params := &xapp.RMRParams{}
	params.Mtype = trans.GetMtype()
	params.SubId = int(instanceId)
	params.Xid = trans.GetXid()
	params.Meid = trans.GetMeid()
	params.Src = ""
	params.PayloadLen = len(trans.Payload.Buf)
	params.Payload = trans.Payload.Buf
	params.Mbuf = nil
	xapp.Logger.Debug("MSG to XAPP: %s %s", trans.String(), params.String())
	sendErr := c.SendWithRetry(params, false, 5)
	if sendErr != nil {
		xapp.Logger.Error("sendSubsDelNotificationToxApp: Send failed: %+v", sendErr.Error())
	}
