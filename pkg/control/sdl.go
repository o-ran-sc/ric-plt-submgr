...
func (c *Control) ReadSubscriptionFromSdl(subId uint32) (*Subscription, error) {
	...
	for _, iSubscriptionInfo := range retMap {
		...
		if subscriptionInfo.SubRespRcvd == "SubResp" {
			...
			subs.SubRespRcvd = false
			subs.SubRFMsg = nil
			xapp.Logger.Debug("SDL: ReadSubscriptionFromSdl() subscriptionInfo.SubRespRcvd == '', InstanceId=%v ", subscriptionInfo.ReqId.InstanceId)
		}
	}
	return subs, nil
}
...
func (c *Control) ReadAllSubscriptionsFromSdl() ([]uint32, map[uint32]*Subscription, error) {
	...
	for _, iSubscriptionInfo := range iSubscriptionMap {
		...
		if subscriptionInfo.SubRespRcvd == "SubResp" {
			...
			subs.SubRespRcvd = false
			subs.SubRFMsg = nil
			xapp.Logger.Debug("SDL: ReadAllSubscriptionsFromSdl() subscriptionInfo.SubRespRcvd == '', InstanceId=%v ", subscriptionInfo.ReqId.InstanceId)
		}
