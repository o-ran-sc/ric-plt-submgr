...
func (c *Control) ReadSubscriptionFromSdl(subId uint32) (*Subscription, error) {
	...
	for _, iSubscriptionInfo := range retMap {
		...
		jsonSubscriptionInfo := iSubscriptionInfo.([]uint8)
		err := json.Unmarshal([]byte(jsonSubscriptionInfo), subscriptionInfo)
		if err != nil {
			return nil, fmt.Errorf("SDL: ReadSubscriptionFromSdl() json.unmarshal error: %v\n", err)
		}
...
func (c *Control) RemoveSubscriptionFromSdl(subId uint32) error {
	...
	if err != nil {
		return fmt.Errorf("SDL: RemoveSubscriptionfromSdl(): %s", err)
	} else {
...
func (c *Control) ReadAllSubscriptionsFromSdl() ([]uint32, map[uint32]*Subscription, error) {
	...
	subscriptionInfos, err := c.db.GetAll()
	if err != nil {
		return nil, nil, fmt.Errorf("SDL: Error while reading subscriptions from DBAAS %v", err)
	} else {
		subs := &Subscription{
			registry: c.registry,
		}
		for _, jsonSubscriptionInfo := range subscriptionInfos {

			subscriptionInfo := &SubscriptionInfo{}
			err := json.Unmarshal([]byte(jsonSubscriptionInfo), subscriptionInfo)
			if err != nil {
				return nil, nil, fmt.Errorf("SDL: ReadAllSubscriptionsFromSdl() json.unmarshal error: %v\n", err)
			}
			...
			if err != nil {
				return nil, nil, fmt.Errorf("SDL: ReadAllSubscriptionsFromSdl() error: %v\n", err)
			}
		}
	}
...
func (c *Control) RemoveAllSubscriptionsFromSdl() error {
	...
	err := c.db.RemoveAll()
	if err != nil {
		return fmt.Errorf("SDL: RemoveAllSubscriptionsFromSdl(): %s", err)
	} else {
