...
func (r *Registry) CreateRESTSubscription(restSubId *string, xAppRmrEndPoint *string, maid *string) (*RESTSubscription, error) {
	...
	/*
		for _, restSubscription := range r.restSubscriptions {
			if restSubscription.xAppRmrEndPoint == *xAppRmrEndPoint && restSubscription.SubReqOngoing == true {
				err := fmt.Errorf("Registry: REST request processing is still ongoing for the endpoint=%s, restSubId=%v", *xAppRmrEndPoint, *restSubId)
				return nil, err
			}
		}
	*/
	...
	if restSubscription, ok := r.restSubscriptions[restSubId]; ok {
		/*
			...
			if restSubscription.SubDelReqOngoing == false && restSubscription.SubReqOngoing == false {
				...
				return restSubscription, fmt.Errorf("Registry: REST request processing is still ongoing for the endpoint=%v, restSubId=%v", restSubscription, restSubId)
			}
		*/
		return restSubscription, nil
	}
