...
func (d *duplicateCtrl) SetMd5sumFromLastOkRequest(restSubsId string, md5sum string) {
	...
	if exists {
		xapp.Logger.Error("Storing md5sum for a processed request for restubsId %s md5sum %s over a previous restSUbsId %s", restSubsId, md5sum, prevRestSubsId)
	} else {
		xapp.Logger.Debug("Storing md5sum for a processed request for restubsId %s md5sum %s", restSubsId, md5sum)
	}
