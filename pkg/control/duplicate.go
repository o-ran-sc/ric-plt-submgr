...
func (c *duplicateCtrl) IsDuplicateToOngoingTransaction(restSubsId string, payload interface{}) (error, bool, string) {
	...
	if present {
		xapp.Logger.Info("Collision detected. REST subs ID %s has ongoing transaction with MD5SUM : %s\n", entry.restSubsId, md5sum)
		c.collCount++
...
func (c *duplicateCtrl) TransactionComplete(md5sum string) error {
