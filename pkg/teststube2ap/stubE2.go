...
func (tc *E2Stub) SendSubsReq(t *testing.T, rparams *E2StubSubsReqParams, oldTrans *RmrTransactionId) *RmrTransactionId {
	...
	err, packedMsg, msgString := e2SubsReq.Pack(myparams.Req, debugPrint)
	if err != nil {
		tc.TestError(t, "pack NOK %s %s", trans.String(), err.Error())
		return nil
	}
	tc.Logger.Debug("%s %s", trans.String(), msgString)
...
func (tc *E2Stub) RecvSubsReq(t *testing.T) (*e2ap.E2APSubscriptionRequest, *xapptweaks.RMRParams) {
	...
	if msg != nil {
		if msg.Mtype != xapp.RICMessageTypes["RIC_SUB_REQ"] {
			...
			unpackerr, req, _ := e2SubsReq.UnPack(packedData, debugPrint)
			if unpackerr != nil {
...
//
//-----------------------------------------------------------------------------
func (tc *E2Stub) GetRESTSubsReqReportParams(subReqCount int, parameterSet int, actionDefinitionPresent bool, actionParamCount int) *RESTSubsReqReportParams {
	...
	if parameterSet == 1 {
		...
		reportParams.GetRESTSubsReqReportParams2(subReqCount, actionDefinitionPresent, actionParamCount, &tc.clientEndpoint, &tc.meid)
	} else {
		tc.Logger.Error("Invalid parameterSet=%v", parameterSet)
	}
...
func (p *RESTSubsReqReportParams) GetRESTSubsReqReportParams2(subReqCount int, actionDefinitionPresent bool, actionParamCount int, clientEndpoint *string, meid *string) {
	...
	for requestCount := 0; requestCount < subReqCount; requestCount++ {
		eventTrigger := clientmodel.EventTrigger{}
		eventTrigger.TriggerNature = "now" // "now" or "on change"
		p.ReportParams.EventTriggers = append(p.ReportParams.EventTriggers, &eventTrigger)
	}
}
...
func (p *RESTSubsReqReportParams) ClientEndpoint(clientEndPoint string) {
	p.ReportParams.ClientEndpoint = &clientEndPoint
}

func (p *RESTSubsReqReportParams) SetTventTriggerTypeOfMessage(typeOfMessage int64) {
	for i := 0; i < len(p.ReportParams.EventTriggers); i++ {
		p.ReportParams.EventTriggers[i].TypeOfMessage = typeOfMessage
	}
}
